...
class AnnouncementsPage extends StatelessWidget {
  ...
  List<Widget> _buildBodyBasedOnState(AnnouncementState state, ThemeData theme) {
    if (state is AnnouncementLoading) {
      return [const SliverFillRemaining(child: Center(child: CircularProgressIndicator()))];
    }
    if (state is AnnouncementLoaded) {
      if (state.announcements.isEmpty) return [_buildEmptyState("announce.no_data".tr())];
      return [_buildAnnouncementsList(state.announcements)];
    }
    if (state is AnnouncementError) return [_buildErrorState(state.message)];
    return [SliverFillRemaining(child: Center(child: Text("announce.preparing".tr())))];
  }
    ...
    return SliverAppBar(
      ...
      flexibleSpace: FlexibleSpaceBar(
        background: Container(
          decoration: BoxDecoration(gradient: LinearGradient(colors: [colorScheme.primary, colorScheme.primaryContainer])),
          child: Padding(
              ...
              children: [
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      Text("announce.title".tr(), style: theme.textTheme.headlineSmall?.copyWith(color: Colors.white, fontWeight: FontWeight.bold)),
                      Text("announce.subtitle".tr(), style: theme.textTheme.bodySmall?.copyWith(color: colorScheme.secondary.withOpacity(0.9))),
                    ],
                  ),
                ),
              ],
    ...
    return SliverPadding(
      ...
      sliver: SliverList(
        delegate: SliverChildBuilderDelegate(
          (context, index) => _buildAnnouncementCard(context, announcements[index]),
  ...
  Widget _buildAnnouncementCard(BuildContext context, AnnouncementModel announcement) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        color: theme.cardColor, borderRadius: BorderRadius.circular(22),
      ...
      child: Material(
        ...
        child: InkWell(
          ...
          onTap: () => context.push('/admin/announcement-details', extra: announcement),
          child: Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      Row(
                        ...
                        children: [
                          Expanded(child: Text(announcement.title, style: theme.textTheme.titleMedium?.copyWith(fontWeight: FontWeight.bold, color: theme.colorScheme.primary))),
                          Icon(context.locale.languageCode == 'ar' ? Icons.arrow_back_ios : Icons.arrow_forward_ios, size: 12, color: Colors.grey),
                        ],
                      ),
                      const SizedBox(height: 10),
                      Text(announcement.description, maxLines: 2, overflow: TextOverflow.ellipsis, style: theme.textTheme.bodyMedium?.copyWith(color: Colors.grey[600])),
                      const Padding(padding: EdgeInsets.symmetric(vertical: 12), child: Divider(thickness: 0.6)),
                      Row(
                        children: [
                          Icon(Icons.group_outlined, size: 18, color: statusColor),
                          const SizedBox(width: 6),
                          Text("${announcement.applicants} ${'announce.details.person_unit'.tr()}", style: theme.textTheme.labelMedium?.copyWith(fontWeight: FontWeight.w600)),
                          const Spacer(),
                          Container(
                            padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 6),
                            decoration: BoxDecoration(color: statusColor.withOpacity(0.12), borderRadius: BorderRadius.circular(10)),
                            child: Text("announce.${announcement.status.toLowerCase()}".tr(), style: TextStyle(color: statusColor, fontSize: 11, fontWeight: FontWeight.bold)),
                          ),
                        ],
                      ),
                    ],
  ...
  Widget _buildEmptyState(String msg) => SliverFillRemaining(
    child: Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [Icon(Icons.inbox_rounded, size: 60, color: Colors.grey[300]), const SizedBox(height: 10), Text(msg, style: const TextStyle(color: Colors.grey))])),
  );

  Widget _buildErrorState(String errorCode) => SliverFillRemaining(
    child: Padding(padding: const EdgeInsets.all(20), child: Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [const Icon(Icons.wifi_off_rounded, size: 60, color: Colors.redAccent), const SizedBox(height: 16), Text(errorCode.tr(), textAlign: TextAlign.center, style: const TextStyle(color: Colors.redAccent))]))),
  );
  ...
  Widget _buildFAB(BuildContext context, ColorScheme colorScheme) {
    return FloatingActionButton.extended(
      ...
      label: Text("announce.add_button".tr(), style: const TextStyle(fontWeight: FontWeight.bold, color: Colors.white)),
    );
  }
}
...
class AnnouncementDetailsPage extends StatelessWidget {
              ...
              background: Stack(
                ...
                children: [
                  if (announcement.imageUrl != null)
                    CachedNetworkImage(
                      imageUrl: announcement.imageUrl!, fit: BoxFit.cover,
                      placeholder: (context, url) => Container(color: Colors.black12),
                      errorWidget: (context, url, error) => Container(color: colorScheme.primary.withOpacity(0.1), child: Icon(Icons.broken_image_outlined, color: colorScheme.primary, size: 40)),
                    ),
                  Container(
                    decoration: BoxDecoration(
                      gradient: LinearGradient(
                        begin: Alignment.topCenter, end: Alignment.bottomCenter,
                        colors: [Colors.black.withOpacity(announcement.imageUrl != null ? 0.3 : 0.0), colorScheme.primary.withOpacity(announcement.imageUrl != null ? 0.8 : 1.0)],
                      ),
                    ),
                  ),
                  Padding(
          ...
          SliverPadding(
            ...
            sliver: SliverList(
              delegate: SliverChildListDelegate([
                _buildAnnouncementDetailCard(context, announcement: announcement),
  ...
  Widget _buildAnnouncementDetailCard(BuildContext context, {required AnnouncementModel announcement}) {
    final theme = Theme.of(context);
    final colorScheme = theme.colorScheme;
    final statusColor = getAnnouncementStatusColor(announcement.status, colorScheme);
    ...
    return Container(
      decoration: BoxDecoration(
        color: Theme.of(context).colorScheme.surface, borderRadius: BorderRadius.circular(28),
          ...
          Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                const Padding(padding: EdgeInsets.symmetric(vertical: 30), child: Divider(thickness: 0.8)),
